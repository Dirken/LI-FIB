...
alreadyPlayed :-
    ...
    findall(match-S-T-R, round(R), Visitor),
    union(Home, Visitor, Lits),
    exactly(1,Lits),
    fail.
alreadyPlayed.
...
doubleRestriction:- 
    ...
    R > 0, 
    R1 is R - 1, 
    expressOr(\+doubleAway-T-R,[home-T-R, home-T-R1]), 
    expressOr(\+doubleHome-T-R,[\+home-T-R, \+home-T-R1]), expressOr(double-T-R, [doubleAway-T-R, doubleHome-T-R]), fail.
doubleRestriction.
...
onlyOneDouble:- team(T), findall(double-T-R, round(R), Lits), atMost(1, Lits), fail.
onlyOneDouble.
