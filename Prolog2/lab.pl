...
% - Every weekday (5 days) has 5 teaching hours (9:00 - 14:00).                                                             DONE
% - Each course has 3 teaching hours per week, distributed in 3 slots of one hour in 3 different days.                      DONE
% - For each student we know the courses s(he) has to attend. No overlapping can exist among these courses.                 DONE 
% - For each course we know the list of rooms where it can be held. All sessions of a course are always in the same room.   DONE
% - Each student has at most 3 hours per day.                                                                               DONE
% - The goal is to design a schedule that satisfies all previous constraints and minimizes                                  DONE  
%the number of days with some lecture from 13:00 to 14:00.
...
threeHoursPerWeek :-
    course(C),
    findall(courseHour-C-D-H, (day(D), hour(H)), Res),
    exactly(3, Res),
    fail.
threeHoursPerWeek.

%distributed in 3 slots of one hour in 3 different days.
oneHourPerDay :-
    course(C),
    day(D),
    findall(courseHour-C-D-H,hour(H),Res),
    atMost(1,Res),
    fail.
oneHourPerDay.
...
studentMost3 :-
    day(D), % en un día
    student(S), % un estudiante
    student(S,A), % con un conjunto de cursos
    findall(courseHour-C-D-H,(member(C,A),hour(H)),Res), 
    atMost(3,Res),
    fail.
studentMost3.
...
assignCourse :-
    course(C),
    courseRooms(C,A),
    findall(courseRoom-C-R, member(R,A), Res),
    exactly(1,Res),
    fail.
assignCourse.
...
% Specific courses can't be hold at the same time
noOverlapping :- 
    student(S), 
    student(S, A), 
    day(D), 
    hour(H), 
    findall(courseHour-C-D-H, member(C,A), Res),
    atMost(1, Res),
    fail.
noOverlapping.
...
noRoomOverlapping :-
    ...
    day(D), 
    hour(H),
    atMost(3, [courseHour-C1-D-H, courseHour-C2-D-H, courseRoom-C1-R, courseRoom-C2-R]),
    fail.
noRoomOverlapping.

defineLD :-
    day(D),
    findall(courseHour-C-D-5, course(C), List),
    expressOr(late-D, List),
    fail.
defineLD.

%He hecho con una función que va probando porque creo que es como sería si el valor fuera 
% 50 o 500 digamos aunque claro
minimizeLD :-
    between(1,5,K),
    findall(late-D, day(D), Res),
    atMost(K,Res),
    fail.
minimizeLD.
