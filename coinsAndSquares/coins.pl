...
main:- 
    ...
    reverse(Coins, X),
    assignCoins(Amount, X, [], Answer),
    nl, write(Answer), nl,nl, halt.

reverse([],Z,Z).
reverse([H|T],Z,Acc) :- reverse(T,Z,[H|Acc]).

concatenation(X, Lista, [X|Lista]).

assignCoins(_, [], _, _).
assignCoins(Amount, [C|CS], Partial, Answer):-
	C =< Amount,
	Aux is Amount div C,
	Ans is Amount - C*Aux,
	concatenation(Aux, Partial, Answer),
	assignCoins(Amount, CS, Answer, Answer),
	fail.
assignCoins(_,_,_,_).
